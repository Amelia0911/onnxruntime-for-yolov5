...
def check_img_size(img_size, s=32):
    ...
    new_size = make_divisible(img_size, int(s))  # ceil gs-multiple，确保能够被32整除
...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False):
    ...
    else:  # transform from xywh to xyxy （cx,cy,w, H）
        b1_x1, b1_x2 = box1[0] - box1[2] / 2, box1[0] + box1[2] / 2  # x方向上的x1, x2
        b1_y1, b1_y2 = box1[1] - box1[3] / 2, box1[1] + box1[3] / 2  # y方向上的y1, y2
        b2_x1, b2_x2 = box2[0] - box2[2] / 2, box2[0] + box2[2] / 2
        b2_y1, b2_y2 = box2[1] - box2[3] / 2, box2[1] + box2[3] / 2

    # Intersection area #求交集-终点坐标最小值，初始坐标最大值,宽度和高度相乘
    ...
    union = (w1 * h1 + 1e-16) + w2 * h2 - inter  # 并集

    iou = inter / union  # iou
    if GIoU or DIoU or CIoU:
        cw = torch.max(b1_x2, b2_x2) - torch.min(b1_x1, b2_x1)  # convex (smallest enclosing box) width，最小闭包宽度
        ch = torch.max(b1_y2, b2_y2) - torch.min(b1_y1, b2_y1)  # convex height，最小闭包高度
        if GIoU:  # Generalized IoU https://arxiv.org/pdf/1902.09630.pdf广义IOU
            c_area = cw * ch + 1e-16  # convex area
            return iou - (c_area - union) / c_area  # GIoU取值范围[-1,1],GIoU不仅关注重叠区域，还关注其他的非重合区域，能更好的反映两者的重合度。
        if DIoU or CIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1#距离iou
            # convex diagonal squared
            c2 = cw ** 2 + ch ** 2 + 1e-16  # 闭包的对角距离平方
            # centerpoint distance squared
            rho2 = ((b2_x1 + b2_x2) - (b1_x1 + b1_x2)) ** 2 / 4 + ((b2_y1 + b2_y2) - (
            b1_y1 + b1_y2)) ** 2 / 4  # box中心点的距离平方
            if DIoU:
                return iou - rho2 / c2  # DIoU：将目标与anchor之间的距离，重叠率以及尺度都考虑进去，使目标回归更稳定
            elif CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                v = (4 / math.pi ** 2) * torch.pow(torch.atan(w2 / h2) - torch.atan(w1 / h1), 2)  # 用来度量长宽比的相似性
                with torch.no_grad():
                    alpha = v / (1 - iou + v)  # 用来做权衡的参数，权重系数
...
def non_max_suppression_test(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False):
    ...
    xc = prediction[..., 4] > conf_thres  # candidates，置信度取出，阈值化

    # Settings
    min_wh, max_wh = 2, 4096  # (pixels) minimum and maximum box width and height，目标尺寸(像素值)
    max_det = 300  # maximum number of detections per image,，每张图像的最大目标数
    time_limit = 10.0  # seconds to quit after
    redundant = True  # require redundant detections
    multi_label = nc > 1  # multiple labels per box (adds 0.5ms/img)，判断是否大于1类
    #     multi_label=False

    t = time.time()
    output = [None] * prediction.shape[0] #初始化输出列表
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Compute conf
        x[:, 5:] *= x[:, 4:5]  # conf = obj_conf * cls_conf，置信度计算，目标得分×类别得分
        ...
        if multi_label:
            i, j = (x[:, 5:] > conf_thres).nonzero().t() #阈值化，输出满足的box的id
        ...
        # tmpx=x[i]  #所有类别做一次iou=0.9的过滤
        # c = tmpx[:, 5:6] * 0 # classes
        # boxes, scores = tmpx[:, :4] + c, tmpx[:, 4]  # boxes (offset by class), scores
        # i = torchvision.ops.boxes.nms(boxes, scores, 0.75)
        # output[xi]=tmpx[i]
        #
        # tmpx=tmpx[i]
        #
        ...
        for class1 in range(len(classes)):
            for class2 in range(len(classes)):
                if scores[class1] > 0.1 and scores[class2] > 0.1 and class1 != class2 and classes[class1] == classes[
                    class2] and ioumin[class1][class2] > 0.85:
